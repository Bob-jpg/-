...
def sav2Imglist(image_results,imagename, output_dir="output"):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for idx, (org_img, result) in enumerate(image_results):
        # 处理图像和结果
        result = result[0]
        image = org_img[:, :, ::-1]  # BGR to RGB
        boxes = [line[0] for line in result]
        txts = [line[1][0] for line in result]
        scores = [line[1][1] for line in result]
        ...
        # 保存图像
        save_path = os.path.join(output_dir, f"draw_ocr_{imagename[idx]}.jpg")
